...
class BusinessCardShapeModel(object):

    class_labels = ["rounded", "squared"]

    def __init__(self):
        yaml_file = open('../Model/bc_shape_classification_vgg16_model.yaml', 'r')
        loaded_model_yaml = yaml_file.read()
        yaml_file.close()
        self.vgg16 = model_from_yaml(loaded_model_yaml)
        self.vgg16.load_weights("../Model/bc_shape_classification_vgg16.h5")

        yaml_file = open('../Model/bc_shape_classification_model.yaml', 'r')
        loaded_model_yaml = yaml_file.read()
        yaml_file.close()
        self.classification_model = model_from_yaml(loaded_model_yaml)
        self.classification_model.load_weights("../Model/bc_shape_classification_model.h5")
        self.img_width = 256
        self.img_height = 256
