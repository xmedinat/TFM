...
class LambdaTextPredictorModel(object):

    max_length = 19

    def __init__(self):
        yaml_file = open('caption_pred_model_all.yaml', 'r')
        loaded_model_yaml = yaml_file.read()
        yaml_file.close()
        self.model = model_from_yaml(loaded_model_yaml)
        self.model.load_weights("caption_pred_model_all.h5")
        with open('corpus_all.txt', 'rb') as corpus_file:
